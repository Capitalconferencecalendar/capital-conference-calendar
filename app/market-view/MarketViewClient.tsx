  ...
  return (
    <div className="v3-page">
      <style jsx>{`
        ...
        .v3-readout { position: relative; display: grid; align-items: start; min-height: 344px; height: max-content; padding: 30px 34px 38px; border: 1px solid rgba(213,226,240,.08); border-radius: 22px; background: radial-gradient(circle at 80% 18%,rgba(51,112,160,.26),transparent 36%), linear-gradient(132deg,#030b16 0%,#06182a 43%,#08111f 100%); box-shadow: inset 0 1px 0 rgba(245,250,255,.16), inset 0 -1px 0 rgba(112,146,180,.1), 0 24px 54px rgba(0,0,0,.34), 0 0 48px rgba(144,178,211,.13); }
        .v3-readout:before { content: ""; position: absolute; inset: 0; border-radius: inherit; background: radial-gradient(circle at 78% 40%,rgba(177,216,245,.24),transparent 3%,transparent 4%), radial-gradient(circle at 88% 20%,rgba(177,216,245,.2),transparent 2.5%,transparent 4%), radial-gradient(circle at 70% 24%,rgba(177,216,245,.18),transparent 2%,transparent 4%), radial-gradient(circle at 91% 55%,rgba(177,216,245,.16),transparent 2.5%,transparent 4%), radial-gradient(circle at 63% 58%,rgba(177,216,245,.14),transparent 2%,transparent 4%), radial-gradient(ellipse at 82% 40%,rgba(103,164,211,.28),transparent 43%), linear-gradient(105deg,rgba(255,255,255,.05),transparent 38%); pointer-events: none; }
        .v3-readout:after { content: ""; position: absolute; inset: 0; border-radius: inherit; opacity: .48; pointer-events: none; background-image: radial-gradient(circle,rgba(207,229,248,.5) 0 1px,transparent 1.5px), linear-gradient(118deg,transparent 0 52%,rgba(166,207,238,.2) 52.2%,transparent 52.8%), linear-gradient(146deg,transparent 0 60%,rgba(166,207,238,.16) 60.2%,transparent 60.8%), linear-gradient(24deg,transparent 0 68%,rgba(166,207,238,.15) 68.2%,transparent 68.8%), radial-gradient(ellipse at 84% 42%,transparent 0 32%,rgba(178,215,241,.22) 32.4%,transparent 33.2%,transparent 42%,rgba(178,215,241,.12) 42.4%,transparent 43.2%); background-size: 36px 36px, 100% 100%, 100% 100%, 100% 100%, 100% 100%; background-position: right center; mask-image: linear-gradient(90deg,transparent 0%,rgba(0,0,0,.08) 36%,rgba(0,0,0,.82) 58%,#000 100%); }
        ...
        .v3-mover-row { display: grid; grid-template-columns: minmax(0,1fr) 44px 46px; gap: 8px; align-items: center; padding: 6px 0; border-top: 1px solid rgba(120,168,212,.12); color: #b9cbe0; font-size: 11.5px; }
        .v3-mover-row:first-of-type { border-top: 0; }
        ...
        <main className="v3-main">
          ...
          <section className="v3-section">
            ...
            <div className="v3-market-tape">
              ...
              <div className="v3-mover-grid">
                {moverGroups.map(([title, rows]) => (
                  <div className="v3-mover-table" key={title}>
                    <h3>{title}</h3>
